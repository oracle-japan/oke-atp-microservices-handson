...
resource "oci_core_internet_gateway" "workshop_ig" {
  compartment_id = "${var.compartment_ocid}"
...
resource "oci_core_route_table" "workshop_route_table" {
  ...
  route_rules {
    ...
    network_entity_id = "${oci_core_internet_gateway.workshop_ig.id}"
  }
}
...
resource "oci_core_subnet" "clusterSubnet_1" {
  ...
  security_list_ids = ["${oci_core_vcn.workshop_vcn.default_security_list_id}"]
